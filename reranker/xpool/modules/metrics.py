...
def sim_matrix_inference(text_embeds_per_video_id, vid_embeds_pooled_per_video_id, pooling_type, candidate_mask=None):
    """
    ...
    Args
        ...
        vid_embeds_pooled_per_video_id: depends on pooling_type
        pooling_type: 'avg' or 'transformer'
        candidate_mask: Optional boolean mask [num_vids, max_text_per_vid, num_vids] 
                       where True indicates valid query-video pairs for candidate reranking
    ...
    if candidate_mask is not None:
        sims = sims.masked_fill(~candidate_mask, float(0))
