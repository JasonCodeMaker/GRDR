...
class CLIPTransformer(nn.Module):
    ...
    def forward_video(self, video_data):
        """
        Encode video frames using CLIP image encoder.
        Handles both HuggingFace and custom CLIP implementations.
        
        Args:
            video_data: Video tensor of shape [B*F, C, H, W]
        
        Returns:
            Video embeddings of shape [B*F, D]
        """
        if self.config.huggingface:
            return self.clip.get_image_features(video_data)
        else:
            return self.clip.encode_image(video_data)
