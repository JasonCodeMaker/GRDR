...
python scripts/panda/extract_panda_features.py \
    --split "${SPLIT:-test}" \
    --gpu_id 0 \
    --mode "${MODE:-both}" \
    --checkpoint "$CHECKPOINT" \
    --frames-root "${FRAMES_ROOT:-dataset/Panda-70M-10M/panda_10m_frames}" \
    --annotation-root "${ANNOTATION_ROOT:-dataset/Panda-70M-10M/video_retreival_caption}" \
    --output-root "${OUTPUT_ROOT:-dataset/features/InternVideo2/panda}" \
    --batch_size "${BATCH:-36}" \
