...
When a live-run skill is installed, launch tracked long-running experiment commands through that skill; wrapper-launched runs then follow that skill's adaptive tracking protocol (startup health gate, run-scaled check cadence, verified completion) in place of this section's fixed 10-minute cadence, which remains the default for unwrapped runs. The wrapper also best-effort ensures the local dashboard server before launch so `research_html/live.html` can read `/api/live/*` without putting volatile `outputs/**` writes in the static page reload path. Dashboard server failure is `repair_required` observation debt, not launch failure: record the warning, keep launching the experiment, and monitor truth through `status.json` / `report.py --open`.
