#!/usr/bin/env bash
# B1 baseline: NLT=1, no pseudo queries, c=4096, l=3, full 3-loop chain on Panda 500K subset.
# Apples-to-apples reference for F2 + K1 multi-view cells. See plan.html / experiments / B1.
set -euo pipefail
...
SAVE_PATH="${SAVE_PATH:-$RUNTIME_ROOT/output/GRDR/b1_nlt1_nopseudo}"
CACHE_DIR="${CACHE_DIR:-$RUNTIME_ROOT/cache/b1}"
...
# Activate the 500K subset swap; restore on exit (idempotent)
source "$SCRIPT_DIR/_subset_swap.sh"
subset_swap_activate "$SUBSET_DIR"
trap 'subset_swap_restore' EXIT

train_b1() {
    log "train B1 NLT=1 no-pseudo exp=$EXP_NAME seed=$SEED code_num=$CODE_NUM"
    ...
    python run.py \
        ...
        --num_candidates "$TRAIN_NUM_CANDIDATES" \
        --setting 1 \
...
export_one_beam() {
    ...
    log "export Setting 1 candidates beam=$beam from $checkpoint_path"
    conda activate semantictvr
    python run.py \
...
export_beams() {
    ...
    [[ -n "$checkpoint_path" ]] || { log "no checkpoint found under $SAVE_PATH/panda"; exit 1; }
    CHECKPOINT_PATH="$checkpoint_path"
    log "selected checkpoint: $CHECKPOINT_PATH"
...
main() {
    log "B1 runtime root: $RUNTIME_ROOT  config: code_num=$CODE_NUM NLT=$NUM_LATENT_TOKENS seed=$SEED device=$DEVICE"
    write_manifest
    [[ "${DRY_RUN:-0}" == "1" ]] && { log "dry run"; return 0; }
    if [[ "$EXPORT_ONLY" == "1" ]]; then
        export_beams; write_manifest; log "done B1 export-only"; return 0
    fi
    log "### TRAIN BEGIN ###"; train_b1; log "### TRAIN END ###"
