#!/usr/bin/env bash
# K1 stacked fix: F2 flags + --codebook_seed_all_slots. Flips do_epoch_encode to
# dedup-by-video + return_all=True so the loop-boundary kmeans sees explicit per-slot
# signal across all N slots, not the select(token_idx)-reduced text-pool.
# Runs on the workstation A6000 (or any GPU); same training compute as F2.
set -euo pipefail
...
SAVE_PATH="${SAVE_PATH:-$RUNTIME_ROOT/output/GRDR/k1_f2_kmeans_allslots}"
CACHE_DIR="${CACHE_DIR:-$RUNTIME_ROOT/cache/k1}"
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-K1_s${SEED}}"
WANDB_MODE="${WANDB_MODE:-online}"
EXPORT_ONLY="${EXPORT_ONLY:-0}"
FULL_LOG="$LOG_DIR/K1_${EXP_NAME}.log"
MANIFEST="$RUNTIME_ROOT/manifests/${EXP_NAME}_manifest.json"
TRIGGER_FILE="$RUNTIME_ROOT/manifests/K1_seed${SEED}_canhit100.txt"
CHECKPOINT_PATH=""
...
main() {
    log "K1 runtime root: $RUNTIME_ROOT"
    write_manifest
    [[ "${DRY_RUN:-0}" == "1" ]] && { log "dry run"; return 0; }
    if [[ "$EXPORT_ONLY" == "1" ]]; then export_beams; write_manifest; log "done K1 export-only"; return 0; fi
    log "### TRAIN BEGIN ###"; train_k1; log "### TRAIN END ###"
