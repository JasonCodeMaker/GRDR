#!/usr/bin/env bash
# B3 diagnostic: NLT=4, +pseudo, c=4096, l=3, NO fix flags. Reproduces the
# diagnosed collapse so F2/K1 deltas are interpretable.
set -euo pipefail
...
SAVE_PATH="${SAVE_PATH:-$RUNTIME_ROOT/output/GRDR/b3_nlt4_pseudo_broken}"
CACHE_DIR="${CACHE_DIR:-$RUNTIME_ROOT/cache/b3}"
...
train_b3() {
    log "train B3 NLT=4 +pseudo BROKEN reproducer exp=$EXP_NAME"
...
main() {
    log "B3 runtime root: $RUNTIME_ROOT"
    write_manifest
    [[ "${DRY_RUN:-0}" == "1" ]] && { log "dry run"; return 0; }
    if [[ "$EXPORT_ONLY" == "1" ]]; then export_beams; write_manifest; log "done B3 export-only"; return 0; fi
    log "### TRAIN BEGIN ###"; train_b3; log "### TRAIN END ###"
