#!/usr/bin/env bash
# B3v: NLT=4 + pseudo + NO fix flags @ full 2.15M Panda. Verifies the diagnostic
# package's original claim that NLT=4+pseudo collapses on Panda 2.15M to
# CanHit@100 ~44.59 (vs anchor 95.57). B3 on 500K subset only reached ~93.7,
# which is close to B1 (~95.0) and inconsistent with the 50pp gap reported at
# 2.15M. B3v re-runs the same broken recipe at the original 2.15M scale to
# decide: is the diagnostic's collapse real, or scale-dependent only at 2.15M,
# or measurement drift?
#
# Same config as B3 except:
#   - canonical panda_ret_train{,_addition}.json (2.15M, no subset_swap)
#   - Bunya conda env via _conda_activate.sh (grdr-stage1-gpu)
#   - larger cache / wandb run-name suffix _2150k
set -euo pipefail
...
SAVE_PATH="${SAVE_PATH:-$RUNTIME_ROOT/output/GRDR/b3v_nlt4_pseudo_broken_2150k}"
CACHE_DIR="${CACHE_DIR:-$RUNTIME_ROOT/cache/b3v}"
...
EXP_NAME="${EXP_NAME:-b3v_nlt4_pseudo_broken_2150k_s${SEED}}"
CANDIDATE_DIR="${CANDIDATE_DIR:-$RUNTIME_ROOT/candidates/$EXP_NAME}"
WANDB_PROJECT="${WANDB_PROJECT:-$PACKAGE_ID}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-B3v_2150k_s${SEED}}"
WANDB_MODE="${WANDB_MODE:-online}"
EXPORT_ONLY="${EXPORT_ONLY:-0}"
FULL_LOG="$LOG_DIR/B3v_${EXP_NAME}.log"
MANIFEST="$RUNTIME_ROOT/manifests/${EXP_NAME}_manifest.json"
TRIGGER_FILE="$RUNTIME_ROOT/manifests/B3v_seed${SEED}_canhit100.txt"
CHECKPOINT_PATH=""
...
# NOTE: No _subset_swap.sh — B3v uses the full canonical Panda 2.15M files.

train_b3v() {
    log "train B3v NLT=4 +pseudo NO-FIX @ 2.15M exp=$EXP_NAME (verifying diagnostic claim of collapse to CanHit@100~44.59)"
...
main() {
    log "B3v runtime root: $RUNTIME_ROOT"
    write_manifest
    [[ "${DRY_RUN:-0}" == "1" ]] && { log "dry run"; return 0; }
    if [[ "$EXPORT_ONLY" == "1" ]]; then export_beams; write_manifest; log "done B3v export-only"; return 0; fi
    log "### TRAIN BEGIN ###"; train_b3v; log "### TRAIN END ###"
