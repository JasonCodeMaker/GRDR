#!/usr/bin/env bash
# V validator: winning recipe (F2 alone OR F2 + K1) at FULL 2.15M Panda corpus on
# Bunya H100 SXM via interactive salloc (R-BUNYA-24H). Goal: beat production
# CanHit@100=95.57.
# RECIPE_AUTODETECT.todo: set K1_WIN=1 at launch iff K1 verdict > F2 by >= 1.0 pp;
# else leave K1_WIN unset/0 to run the F2-only recipe.
set -euo pipefail
...
SAVE_PATH="${SAVE_PATH:-$RUNTIME_ROOT/output/GRDR/v_2150k_winner}"
CACHE_DIR="${CACHE_DIR:-$RUNTIME_ROOT/cache/v}"
...
if [[ "$K1_WIN" == "1" ]]; then
    RECIPE="k1"
    K1_FLAG="--codebook_seed_all_slots"
else
    RECIPE="f2"
    K1_FLAG=""
fi
...
# V trains on the FULL 2.15M Panda corpus. No subset swap.

train_v() {
    log "train V recipe=$RECIPE on full 2.15M Panda exp=$EXP_NAME"
...
m = Path(sys.argv[1])
payload = {
    "package": "$PACKAGE_ID", "cell": "V", "recipe": "$RECIPE",
...
main() {
    log "V runtime root: $RUNTIME_ROOT  recipe=$RECIPE  K1_FLAG='$K1_FLAG'"
    write_manifest
    [[ "${DRY_RUN:-0}" == "1" ]] && { log "dry run"; return 0; }
    if [[ "$EXPORT_ONLY" == "1" ]]; then export_beams; write_manifest; log "done V export-only"; return 0; fi
    log "### TRAIN BEGIN ###"; train_v; log "### TRAIN END ###"
